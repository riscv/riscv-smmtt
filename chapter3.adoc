...
which a hart is operating and configuration for supervisor domain-related
extensions. The interface consists of M-mode CSRs `msdcfg` and `mmpt`. Both CSRs
must be implemented; `msdcfg` may be read-only zero if none of the optional
extensions that are configured via fields of this CSR are implemented.
...
as read-only zero. Hence if no optional extensions are implemented, this CSR
becomes read-only zero.
