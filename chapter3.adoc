...
== `Smsd`: Supervisor Domains 

`Smsd` comprises Memory Protection Tables to set the access permissions for a memory
region or page associated with a supervisor domain, described in <<Smmpt>>, 
and an interface to program the configuration for supervisor-domain-related extensions 

The interface consists of M-mode CSRs
`msdcfg` and `mmpt`. When `Smsd` is implemented, both CSRs must be
...
=== M-mode Memory Protection Table Configuration (`mmpt`)
...
`SDIDLEN`, termed `SDIDMAX`, is 6. Bits labelled 0 (NZR) are reserved for future 
standard use and, until its use is defined by some standard extension, must be
zeroed by software for forward compatibility. Implementations without a 
defined use for these bits shall return zero when read
...
specification cite:[ISA]. The field `PPN` must be
set to zeros.
...
`MFENCE.PA` is only valid in M-mode. If operand `rs1 != x0`, contents of register rs1 specifies
a single physical address, and if `rs2 != x0`, contents of register rs2 specifies a single `SDID`.
...
The `MINVAL.PA` instruction supports batching of MPT invalidations,
...
The behavior of the `SFENCE.VMA`, `HFENCE.GVMA`, `HFENCE.VVMA`, `SINVAL.VMA`, `HINVAL.GVMA`,
and `HINVAL.VVMA` instructions are affected when the `Smsd` extension is implemented. 
...
Details of `Smsdia` and `Smsdqosid` are described in
...
{reg: [
  {bits:  6, name:  'SIDN'},
  {bits: 14, name:  'Non Zero Reserved'},
  {bits:  1, name:  'SSRM'},
  {bits:  1, name:  'SSMM'},
  {bits:  4, name:  'SRL'},
  {bits:  4, name:  'SML'},
...
Bits 21:8 are reserved for future standards use and, until its use is defined by some standard extension, must be
zeroed by software for forward compatibility. Implementations without a defined use for these bits shall return zero when read.
