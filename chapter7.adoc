...
The `smsiaddrcfg` register is extended to add a low child index shift `LIXS`
field at bit position 27:24. The `LIXS` field is used in determination of 
MSI target address for a supervisor interrupt domain. 
