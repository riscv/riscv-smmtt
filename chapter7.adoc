...
When `Smifdeleg` is implemented and `msdgcfg.SIDN` identifies an implemented
_supervisor interrupt domain_, each bit of `msideip` corresponding to an
implemented _supervisor interrupt domain_ must be set to one if and only if the
relevant interrupt source is active.

Specifically:

* For bit `msdcfg.SIDN`, the bit is set to one if and only if either:

** The SEIP signal for _supervisor interrupt domain_ `msdcfg.SIDN` is asserted,
   and bit S of `mifd` is set to one; or
...
* Bits for other implemented _supervisor interrupt domains_ are set to one
