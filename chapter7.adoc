...
supervisor interrupt domain implements _guest interrupt files_ then the
...
in the supervisor interrupt domain selected by `msdcfg.SIDN`. 
...
`GEILEN-1:0` shall be writeable in `GIRF`, and all other bit positions shall be
read-only zeros.
...
If the hypervisor extension is also implemented and the supervisor interrupt
...
* Bits `GEILEN:1` of `hgeie` are writable if the bit at same position in `mgirfd`
...
* If `hstatus.VGEIN` is not 0 and the bit selected by `VGEIN` in `mgirfd` is 1
  ...
  selected by `VGEIN` in `mgirfd` is 0 then the `vsiselect`, `vsireg`, and
  `vstopei` CSRs operate as-if the `VGEIN` is the number of an unimplemented
  guest external interrupt.

If the hypervisor extension is also implemented and the supervisor interrupt
domain selected by `msdcfg.SIDN` is not an implemented supervisor interrupt
domain, then:
...
one or more of the interrupt register files (usually _guest interrupt files_) to
a receipient supervisor domain by invoking an SBI functions provided by the RDSM.
...
`hgeie` to be contigous, or expect to use a supervisor-level interrupt file when
not delegated.
