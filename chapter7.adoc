...
The `mifd.S` bit when set to 1 enables access to the supervisor-level interrupt
file in the _supervisor interrupt domain_ selected by `msdcfg.SIDN`.

If the `mifd.S` bit is 1 and the _supervisor interrupt domain_ selected by
...
If the `mifd.S` bit is 0 or the _supervisor interrupt domain_ selected by
