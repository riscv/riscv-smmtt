...
Cpmsyamys `B` and `D` in this formula are specified by cite:[AIA], while `I`
is a new onstant for supervisor interrupt domains. Constant `I` must be
...
The `msdeip` is a 64-bit read-only register, formatted as shown in <<MSDEIP>>.
When MXLEN=32, `msdeiph` is a 32-bit read-only register which aliases bits
63:32 of `msdeip`. When MXLEN=64, `msdeiph` does not exist.
...
The `msdeie` is a 64-bit read-write register, formatted as shown in <<MSDEIE>>.
When MXLEN=32, `msdeieh` is a 32-bit read-write register which aliases bits
63:32 of `msdeie`. When MXLEN=64, `msdeieh` does not exist.
