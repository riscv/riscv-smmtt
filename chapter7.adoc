...
Constants `B`, `C`, and `D` in this later formulas are specified by
...
CSRs, along with an machine supervisor domain external interrupt (`MSDEI`).
...
a machine supervisor domain external interrupt. The enable bits in `msideie` do not
...
[NOTE]
====
The RDSM may use the machine supervisor domain external interrupt to determine if
...
=== "Smifdeleg" : Standard Extension for Delegation of IMSIC Guest Interrupt Files

The `Smifdeleg` extension enables delegating a subset of the guest interrupt
files within a _supervisor interrupt domain_ in an IMSIC to a supervisor domain.

The `Smifdeleg` extension depends on the `Smaia` extension. When `Smifdeleg` is
implemented, the bit 9 of `mviend` must be writable.
...
If the H extension is also implemented, then each bit of the `GIF` field when
set to 1 enables access to corresponding _guest interrupt file_ in the _supervisor
interrupt domain_ selected by `msdcfg.SIDN`. If `GEILEN` is nonzero, bit
...
* Bits `GEILEN:1` of `hgeip` are read-only aliases of the same bits in the
  ...
  position in `mifd` is 1. All other bits of `hgeip` are zero.
...
* Bits `GEILEN:1` of `hgeie` are writable aliases of the same bits in the
  ...
  position in `mifd` is 1. All other bits of `hgeie` are read-only zero.
...
If the _supervisor interrupt domain_ selected by `msdcfg.SIDN` is not implemented
then:
...
The `hip.SGEIP` bit is 1 if and only if the bitwise logical-AND of CSRs `hgeip`,
`hgeie`, and `mifd` is nonzero in any bit.

Each bit __i__ of `msideip` corresponding to an implemented _supervisor interrupt
domain_ must be set to one if and only if the relevant interrupt source is active.

Specifically:

* If __i__ equals `msdcfg.SIDN`, the bit is set to one if and only if either:

** The SEIP signal for _supervisor interrupt domain_ `msdcfg.SIDN` is asserted,
   and bit `mifd.S` is set to one; or
** The bitwise AND of `mifd`, `hgeip`, and `hgeie` for _supervisor interrupt
   domain_ `msdcfg.SIDN` is nonzero (i.e., as `SGEIP` would evaluate for that
   domain).

* Otherwise:

** The SEIP signal for that _supervisor interrupt domain_ is asserted; or
** The bitwise AND of `hgeip` and `hgeie` for that domain is nonzero (i.e., as
   `SGEIP` would evaluate for that domain).

Bits in `msideip` corresponding to unimplemented _supervisor interrupt domains_
must be set to zero.
