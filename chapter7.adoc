...
The `Smirfd` extension enables delegation of the interrupt register files in
the supervisor interrupt domain in an IMSIC to a supervisor domain.

The `Smirfd` extension depends on `Smaia`.

To support the delegation, the extension introduces a MXLEN-bit read-write
...
supervisor interrupt domain, then:

* The VS-level external interrupt pending signals of the guest interrupt files
  of the selected supervisor interrupt domain are indicated in the `hgeip` CSR
  if the bit corresponding to their guest external interrupt number in `mgirfd`
  is 1 else they are indicated as 0.

* Bits `GEILEN:1` of `hgeie` are writable if the bit at same position in in
  `mgirfd` is 1. All other bits are read-only zeroes.

* If `hstatus.VGEIN` is not 0 and the bit selected by `VGEIN` in `mgirfd` is 1
  then the `vsiselect` and `vstopei` CSRs operate on the registers of the guest
  interrupt file selected by `VGEIN`. If `VGEIN` is 0 or the the bit selected by
  `VGEIN` in `mgirfd` is 0 then the `vsiselect` and `vstopei` CSRs operate as-if
  the `VGEIN` is the number of an unimplemented guest external interrupt.

If the supervisor interrupt domain selected by `msdcfg.SIDN` is not an
implemented supervisor interrupt domain, then:

* The `hgeip` and `hgeie` CSRs are read-only zeros.

* The `vsiselect` and `vstopei` CSRs operate as-if the `VGEIN` is the number of
  an unimplemented guest external interrupt for all values of `VGEIN`.
...
[NOTE]
====
The `Smirfd` is intended to be used to delegate interrupt files to enlightened
(a.k.a para-virtualized) supervisor domains. Usually one of the supervisor domain
that participates in the delegation is a donor domain that initially has access
to all the interrupt files. 

Such donor supervisor domain may then donate, either temporarily or permanently,
one or more of the interrupt register files (usually guest interrupt files) to a
receipient supervisor domain by invoking an SBI functions provided by the RDSM.
The donor domain does not expect to access the donated interrupt files once they
have been delegated by the RDSM to the receipient domain.

The recepient domains must not expect the donated interrupt files to be
contiguously laid out in memory, or expect the corresponding bits in `hgeip` and
`hgeie` to be contigous, or expect to use a supervisor-level interrupt file.

The memory location of the delegated interrupt files and their guest external
interrupt numbers are provided to the receipient supervisor domains by the RDSM
through software mechanisms such as SBI and device tree.
====
