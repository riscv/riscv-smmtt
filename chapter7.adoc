...
appropriate `mideleg` bits. This delegation could require the RDSM to
...
enabling a supervisor domain to be allocated an individual _supervisor interrupt domain_.
...
Existing interrupt controllers not aware of supervisor domains can be associated with a single supervisor domain.
...
[NOTE]
====
Not every supervisor domain may require an individual _supervisor interrupt domain_, and the specification supports 
flexible allocation of fewer supervisor interrupt domains among a larger number of supervisor domains..
====
...
The `SIDN` field is a WLRL field where the only legal values are between 0
and the maximum implemented _supervisor interrupt domain number_, inclusive.
SIDN is a 6-bit field and hence supports up to 64 supervisor interrupt domains.
If there is only a single _supervisor interrupt domain_ connected to the hart,
then `SIDN` may be read-only zero.

When the _supervisor interrupt domain_ selected by `msdcfg.SIDN` is an IMSIC,
the following rules apply:

* The supervisor-level external interrupt pending signal of the supervisor-level
  interrupt file is indicated in `mip.SEIP`.
...
* The guest external interrupt pending signals of the _guest interrupt files_
  are indicated in the `hgeip` CSR.
* The `hstatus.VGEIN` selects a _guest interrupt file_ and `vsiselect` and `vstopei` 
  CSRs operate on the registers of the corresponding _guest interrupt file_.

When the harts's interrupt controller is an APLIC, the following rules apply for the supervisor interrupt domain selected by SIDN:

* The supervisor-level external interrupt pending signal is indicated in `mip.SEIP`.

[NOTE]
====
If the number of implemented supervisor interrupt domains is fewer than the number of supervisor domains that need interrupt controllers, 
the RDSM may emulate interrupt controllers for the additional supervisor domains.
An APLIC can be emulated by trapping memory accesses.
...
[NOTE]
====
The RDSM may use the machine supervisor-domain external interrupt (MSDEI) to determine if
...
assigned to it is scheduled, the RDSM updates the `msdcfg.SIDN` to select that supervisor
...
The RDSM may delegate `MSDEI` to a supervisor domain to be notified about supervisor domain external interrupts pending for
...
delegation supports optimizing the exit sequence from such a supervisor domain by
...
The `Smgeien` extension enables access by a supervisor domain to a subset of the guest interrupt
files within a _supervisor interrupt domain_ in an IMSIC.
...
The `mgeien.A` bit, when set to 1, enables `msideip` to reflect the OR of all external
interrupts for the supervisor interrupt domain; otherwise, `msideip` reflects
only the subset of guest external interrupts determined by `mgeien.GIF`. When
`mgeien.A` is 0, the SEIP for the supervisor interrupt domain is reflected in
...
`GEILEN`. When `GEILEN` is nonzero, the H extension requires that
...
* The implemented guest external interrupts and their enables might not be contiguous in
  `hgeip` and `hgeie`, respectively.
* The supervisor-level and guest interrupt files might not be laid out
  contiguously in memory.
* The supervisor-level interrupt file might not be available to a supervisor
