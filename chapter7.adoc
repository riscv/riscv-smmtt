...
The `mifd.A` bit, when set to 1, enables `msideip` to accumulate all guest
external interrupts for the supervisor interrupt domain; otherwise, `msideip`
accumulates only the subset of guest external interrupts determined by
`mifd.GIF`.
...
** The logical AND of `mip.SEIP` and bit 9 of `mvien` (for SEI) is one
   (true); or
...
** The bitwise logical AND of the following three components of the _supervisor
   ...
   *** the _logical complement_ of `mifd` if `mifd.A` is 0; otherwise, a value
       of all 1s.
