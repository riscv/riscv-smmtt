...
=== "Smgirfp" : Standard Extension for partitioning guest interrupt register files

The `Smgirfp` extension enables sharing of guest interrupt register files in the
supervisor interrupt domain in an IMSIC among one or more supervisor domains by 
partitioning the available guest interrupt register files among the supervisor
domains. When such partitioning is used, only one of the supervisor domains
sharing the guest interrupt register files a IMSIC supervisor interrupt domain
may be associated with the supervisor level interrupt file in that domain. 

The `Smgirfp` extension depends on `Smaia`.

To support the partitioning, the extension introduces a MXLEN-bit read-write 
`mgirfp` register formatted for MXLEN=64 as shown in <<MGIRFP>>. When MXLEN=32,
`mgirfph` is a 32-bit read-only register which aliases bits 63:32 of `mgirfp`.
When MXLEN=64, `mgirfph` does not exist.

[[MGIRFP]]
.`mgirfp` register for RV64
...
supervisor interrupt domain selected by `msdcfg.SIDN`. The `GIRF` bits
corresponding to unimplemented Guest interrupt files are read-only zero.

If the `S` bit of `mgirfp` is 1 and the supervisor interrupt domain selected by
...
If the `S` bit of `mgirfp` is 0 then:
...
corresponding to those guest interrupt files in `mgirfp` is 1 else they are
indicated as 0.

If bit of `mgirfp` selected by `hstatus.VGEIN` is 1 and the supervisor interrupt
