...
The `Smgeien` extension depends on the `Smaia`, `Ssaia`, and the H extension. When
`Smgeien` is implemented, the bit 9 of `mvien` must be writable.

This extension introduces an 64-bit read-write register named `mgeien`, formatted as
shown in <<MGEIEN>>. When MXLEN=32, `mgeienh` is a 32-bit read-write register that
aliases bits 63:32 of `mgeien`. When MXLEN=64, the `mgeienh` register does not exist.
...
other bit positions of the `GIRF` field shall be read-only zeros.
...
The `mgeien.A` bit, when set to 1, enables `msideip` to accumulate all guest
...
`mgeien.GIF`. It is strongly recommended that the bit `mgeien.A` be writable.
...
[NOTE]
====
The `Smgeien` extension is intended for use cases involving a pair of supervisor
domains that share the interrupt files in a _supervisor interrupt domain_.
...
explicitly made accessible. This modified behavior is indicated by the `Ssgeien`
extension.
