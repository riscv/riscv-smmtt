...
If the `S` bit of `mirfd` is 0 or the supervisor domain selected by
`msdcfg.SIDN` is not an implemented then:
...
domain_ selected by `msdcfg.SIDN` is an implemented then:
...
If the hypervisor extension is also implemented and the _supervisor interrupt
domain_ selected by `msdcfg.SIDN` is not an implemented then:
