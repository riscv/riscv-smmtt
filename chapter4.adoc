...
7. Determine if the requested memory access is allowed by the _XWR_ bits, given
   ...
   memory access is allowed. The MXR mechanisms or the use of HLVX instruction
   cannot be used to affect or override the interpretation of permissions
   encoded in memory-protection-table entries.
