...
|16    |`operand-0`    |8       |<<      Operand 0    >>  | No
|24    |`operand-1`    |8       |<<      Operand 1    >>  | No
|32    |`operand-2`    |8       |<<      Operand 2    >>  | Yes
|===
...
`operand-*` registers.  The `OP` field also defines the encoding of the
`operand` fields in the `control` register and of the `operand-*` registers for
that operation.
...
{reg: [
  {bits:  8, name: 'OP (WARL)'},
  {bits: 16, name: 'operands (WARL)'},
  {bits:  7, name: 'STATUS (RO)'},
  {bits:  1, name: 'BUSY (RO)'},
  {bits: 32, name: 'operands (WARL)'}
...
| `SET_SDID_ENTRY`  | 3         | Configure MPT and QoS ID parameters for an SD.
| `GET_SDID_ENTRY`  | 4         | Read the configuration associated with an SD.
...
the `BUSY` bit to 1, indicating that IOMPTCHK is executing the requested
operation. The behavior of writing the `control` or the `operand-*` registers
while the `BUSY` bit is 1 is `UNSPECIFIED`. Some implementations may ignore the
second write, while others may perform the operation specified by that write.
Software must verify that `BUSY` is 0 before writing to the `control` or
`operand-*` registers.
...
provides a status value (<<IOMPTCHK_STS>>) for the requested operation.
...
program the fields of the `operand-0`, `operand-1`, and `operand-2` registers as
