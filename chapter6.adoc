...
* If the update to a field of the MPT entry will make the field inconsistent
  ...
  and use the `MPTINVAL` operation outlined in <<CMD>> to invalidate previous
...
* I/O MPT Checker is not required to immediately observe the software updates to
  an MPT entry. Software must use the `MPTINVAL` operation outlined in <<CMD>>
...
To support MPT enforcement, the RDSM programs two classes of configuration into the I/O MPT Checker:


* Supervisor Domain Classification (SDCL) rules, which classify DMA transactions to a supervisor domain identifier
(SDID) and, optionally, an IOMMU instance. Classification may use implementation-defined transaction attributes 
such as a device identifier, requester identifier, PCIe IDE stream identifier, TEE-association information, or 
other source-identification metadata carried with the transaction.

* Supervisor-domain configuration entries, which associate each SDID with the MPT configuration used to perform access 
checks for transactions originating from that supervisor domain, and with any QoS identifiers that accompany such memory requests.

A transaction is TEE-associated when it originates from a device context that is assigned to a trusted execution 
environment and carries platform-defined metadata indicating that association. SDCL rules may distinguish between 
TEE-associated and non-TEE-associated transactions originating from the same device or communication channel.
...
The reset value for `control.MODE` must reset to `Off`, unless a platform defines alternative behavior.
...
|===
|`STATUS` | Description
| 0       | No status available; reason UNSPECIFIED.
...
The `status.CODE` value of `0` is designated as the reset default. If a value of
`0` is returned in response to a command execution, the command should be
considered to have failed for UNSPECIFIED reasons. It is recommended that this
status code be returned only if all other defined status codes are determined to
be inapplicable.
...
{reg: [
  ...
  {bits: 8, name: 'BARE_IOMMU_ID(WARL)'},
  {bits : 4, name: 'BARE_QRID(WARL)'},
...
as described in <<IOMPTABORT>>. It also ensures that all transactions and
previous requests from devices that have already been processed by the I/O MPT
Checker are committed to a global ordering point such that they can be observed
by all RISC-V harts, devices, I/O MPT Checkers, and IOMMUs in the platform.
...
When the `MODE` is `Bare`, SDCL rules are not consulted for inbound transactions,
and the following rules apply:

* The I/O MPT Checker shall request the I/O Bridge to allow inbound transactions that are 
  not TEE-associated and to abort those that are TEE-associated.

* For allowed transactions, the I/O Bridge shall use the IOMMU instance identified by 
  `control.BARE_IOMMU_ID` to perform address translation. 
  
* If any application processor hart in the system supports the Ssqosid extension, the 
  `QRID` associated with the transaction shall be the value held in `control.BARE_QRID`. 
  
* If the implementation does not include an IOMMU, writing to `BARE_IOMMU_ID` is ignored, and reads 
  return an `UNSPECIFIED` value.  In this case, the mechanism used to determine `RCID` and `MCID` associated with 
  transactions processed while `MODE` is `Bare` is implementation defined.  
  

The `BARE_IOMMU_ID` field must contain a value in the range 0 to N–1, where N is the maximum number 
of IOMMU IDs supported by the implementation. Programming a value outside this range is UNSPECIFIED.
Software must program `BARE_IOMMU_ID` with a valid IOMMU ID before transitioning `MODE` to `Bare`.
Additionally, if `MODE` resets to `Bare`, `BARE_IOMMU_ID` must reset to a valid IOMMU ID, and if any 
application processor hart in the system supports the Ssqosid extension the `BARE_QRID` must
reset to a valid `QRID`.
...
performed as little-endian accesses and when 1 as big-endian accesses. MBE can be 
hardwired when only one endianness is supported.
...
IOMMU. If there is no IOMMU, then the source of the RCID-value and MCID-value is
implmentation defined. These fields shall be implemented if any application processor hart in
...
The `SQRID` field holds the `QRID` associated with a supervisor domain.
The `SQRID`, together with the effective `RCID` and `MCID`, accompanies
requests originating on behalf of that supervisor domain. The `SQRID`
identifies the QRI used by shared resource controllers to interpret the
associated `RCID` and `MCID` values.

Memory accesses generated by an IOMPT page-table walk shall use the
`SQRID` of the supervisor domain on whose behalf the walk is performed,
together with the effective `RCID` and `MCID` for that
supervisor domain. The effective `RCID` and `MCID` shall be
determined using the rules defined above for IOMMU-originated requests.
...
by I/O MPT Checker are committed to the global ordering such that they can be
