...
`control.RULEID` is returned in the `data-0` register.
...
The interpretation of the `control` and `data-0` registers for this operation is
...
supervisor domain in MPTCHK. This operation uses the operands in the `control`,
`data-0`, and `data-1` registers. The fields of these registers are interpreted
as follows.

.`control` register layout for `SET_SDCFG_ENTRY` operation
[wavedrom, , ]
....
{reg: [
  {bits:  8, name: 'OP (WARL)'},
...
identified by `control.SDID` is returned in the `data-0` register.
...
The interpretation of the `control`, `data-0`, and `data-1` registers for this
...
.`control` register layout for `MPTINVAL` operation
[wavedrom, , ]
....
{reg: [
  {bits:  8, name: 'OP (WARL)'},
...
operation. For this operation, the fields of the `control` register are
interpreted as follows.

.`control` register layout for `IOFENCE` operation
[wavedrom, , ]
....
{reg: [
  {bits:  8, name: 'OP (WARL)'},
