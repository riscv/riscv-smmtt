...
`SRC_ID`, `IOMMU_ID`, `IOSDID`, `MPT_MODE`, `SSRM`, `SSMM`, `SRL`, `SML`,
`SQRID` and `PPN`.
...
`SML`, `SQRID`, and `PPN`. The state of all other fields in the `operand-0`
and `operand-1` registers is `UNSPECIFIED`.
...
The `SRL` and `SML` fields along with `operand-1.SSRM` and `operand-1.SSMM` fields
are used to determine the effective `RCID` and `MCID` provided by the IOMMU for
device originated requests. The determination of the effective `RCID` and `MCID`
is as specified by <<SMQOSID>>. The `SQRID` identifies the QRI for requests
originating from the devices and the IOMMU associated with the SD and accompanies
the `RCID` and `MCID` in the requests made by the device to the QRI.
