...
* For allowed transactions, the I/O Bridge shall use the IOMMU instance identified by 
  `control.BARE_IOMMU_ID` to perform address translation. If the implementation does not 
  include an IOMMU, `BARE_IOMMU_ID` is read-only zero and address translation is not performed. 
