...
* SDCL is not configured with a rule to classify a transaction.
* Memory access is disallows by the MPT
* Access to the MPT itself is disallowed by a physical memory attributes check
  or physical memory protection check.
* Access to the MPT returns corrupted (e.k.a poisoned) data.
...
|4     |`control`      |4       |<<CTL, Control          >>  | No
|4     |`command`      |4       |<<CMD, Command          >>  | No
...
If software changes the values of field besides `MODE` field in this register
when the `MODE` is not `Off`, then the I/O MPT checker behavior is UNSPECIFIED>
When the state is `On`, software must first change the `MODE` to `Off` without
changing the values of any other field.
