...
check is provided on the MPT check completion (MCC) interface to the IO Bridge.
...
continue providing service, the I/O MPT checker may request the IO bridge to
...
If a transaction does not match any of the rules, the IO Bridge is notified of
this condition. The subsequent behavior of the IO Bridge for unmatched
transactions remains UNSPECIFIED.
