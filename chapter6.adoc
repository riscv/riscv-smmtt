...
* For allowed transactions, the I/O Bridge shall use the IOMMU instance identified by 
  `control.BARE_IOMMU_ID` to perform address translation. The `QRID` associated with the transaction 
  shall be the value held in `control.BARE_QRID`. If the implementation does not 
  include an IOMMU, `BARE_IOMMU_ID` is read-only zero, address translation is not performed, and the 
  the `RCID` and `MCID` associated with transactions processed while `MODE` is `Bare` are `UNSPECIFIED`.  
...
Additionally, if `MODE` resets to `Bare`, `BARE_IOMMU_ID` must reset to a valid IOMMU ID.
...
Memory accesses generated by an IOMPT page-table walk shall use the effective
`SQRID`, `RCID`, and `MCID` associated with the supervisor domain on whose
behalf the walk is performed. The effective `RCID` and `MCID` shall be
