...
a 64-bit read/write register called `msrmcfg`. The register is formatted as
shown in <<MSRMCFG64>>. When MXLEN=32, `msrmcfgh` is a 32-bit read/write
register that aliases bits 63:32 of `msrmcfg`. The `msrmcfgh` register does not
exist when MXLEN=64.
...
The `SRL` (supervisor-RCID-length) and `SML` (supervisor-MCID-length) fields
in the `msrmcfg` register specify the usable range of `RCID` and `MCID`. The
`SRL` field is WARL with legal values from 0 to `RCIDLEN` where `RCIDLEN`
is the number of implemented bits of the `RCID` field of `srmcfg`. The `SML`
field is WARL with legal values from 0 to `MCIDLEN` where `MCIDLEN` is the
number of implemented bits of the `MCID` field of `srmcfg`.

The supervisor-srmcfg-rcid-mode (`SSRM`) and `SRL` fields determine the legal
values for the `srmcfg.RCID` field. When `SSRM` is 0, the legal values are
between 0 and `2^SRL^ - 1`. When `SSRM` is 1, the legal values are between
0 and `2^RCIDLEN^ - 2^SRL^ - 1` where `RCIDLEN` is the number of implemented
bits of the `srmcfg.RCID` field.
...
0 and `2^MCIDLEN^ - 2^SML^ - 1` where `MCIDLEN` is the number of implemented
bits of the `srmcfg.MCID` field.
...
The RDSM is expected to configure the `msrmcfg` fields such that each SD is
assocaited with a disjoint range of values for `RCID` and `MCID`.
