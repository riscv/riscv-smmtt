...
With supervisor domains, resource-usage information associated with an SD or the RDSM 
shall not be visible through resource monitoring mechanisms to entities outside that SD's 
or RDSM's TCB. Similarly, resource allocation policies and limits associated with an SD or 
the RDSM shall not be modifiable or influenceable by entities outside the corresponding TCB
...
The Smqosid introduces a read/write M-mode CSR (`mrmcfg`) to configure
QoS identifiers for requests made by a hart with effective privilege mode equal
to M.

The `mrmcfg` register is an MXLEN-wide WARL read/write machine-mode CSR used to
...
When Smqosid is implemented, the configurations in the `srmcfg` CSR
...
introduces a MXLEN-wide WARL read/write machine-mode register called `mnrmcfg`.
...
or more memory-mapped QoS register interfaces (QRIs). A QRI may be made
...
The number of `RCID` and `MCID` supported by the controllers for different QRI need
...
When a controller supports only a single QRI, M-mode software is required to
...
by M-mode software. This QRI is known as the __machine-mode QRI__ (MQRI).
...
exclusively by M-mode software or may be shared with one or more supervisor
domains.
