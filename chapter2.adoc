...
memory isolation along with mechanisms such as `e(PMP)`. Device side
...
accessible to lower privilege levels). Other memory access control mechanisms
may also be used to enforce supervisor domain isolation for use cases that do
not require the full flexibility of MTT. For example, (e)PMP configurations
controlled by M-mode during domain context switching. In this case the MTT-mode
`Bare` may be used. When MTT mode is `Bare`, MTT is not enforced, but the SDID
